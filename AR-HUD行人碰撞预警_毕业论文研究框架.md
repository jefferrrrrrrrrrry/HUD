...
>
> **v2 修订说明（2026-08）**：文献库已由 40 篇扩充至 **102 篇**；实验结构由四实验改为 **一个标定实验（实验 0）+ 四个主实验**；核心时间自变量由绝对 TTC 改为 **相对提前量 $\Delta t$ × 触发准则类型**，被映射的风险量统一为 **BTN**。完整论证见 [优化方案](优化方案_预警时间参数理论化重构与执行计划.md)，各实验的最终设计见 [实验0](实验0_自发察觉基线标定_文献与设计依据.md)、[实验1](实验1_单层时间参数筛选_文献与设计依据.md) §10、[实验2](实验2_分层预警升级规则_文献与设计依据.md) §7、[实验3](实验3_空间参照系与锁定策略_文献与设计依据.md) §8、[实验4](实验4_风险演化动态_文献与设计依据.md) §9。
...
本论文拟通过一个标定实验与四个递进主实验，先标定驾驶员在无辅助条件下的自发察觉基线（实验 0），再分离验证 AR-HUD 行人碰撞预警的出现时机、持续方式与分层升级规则，再检验空间参照系和风险演化动态对驾驶员风险加工的促进作用，最终形成具有明确证据边界的时空设计规范。

### 0.2 核心贡献、证据与边界

| 论证要素 | 本论文的处理方式 |
|---|---|
| 研究缺口 | 现有研究多比较“有/无 AR”或单一图形，缺少时间参数、级间间隔与空间动态机制的正交验证 |
| 核心推进 | 将问题拆为“何时显示—何时升级—锁定哪里—如何变化”四个可检验环节 |
| 决定性证据 | 实验 0 标定的 $t_0$／PRT／$a_{req}$ 可行域，加上四个主实验的行为、眼动、安全和认知代价指标，以及实验间参数固定形成的递进验证 |
| 结论边界 | 短期模拟驾驶、视觉 AR-HUD、城市道路行人横穿；不直接外推到真实道路长期使用或最后时刻的多模态干预 |
| 设计原则 | 不强行选出“冠军条件”；安全非劣且收益具有实际意义时才推荐，并列时选择更简单、侵入性更低的方案 |
...
| $\Delta t$ | **相对提前量**（relative lead time）；$\Delta t = t_0 - t_{warn}$，本文实验 1 的核心自变量 |
...
**AR-HUD 行人碰撞预警的时空参数设计及其对驾驶员风险加工与避险绩效的影响**
...
围绕 AR-HUD 行人碰撞预警中的两个核心问题建立可检验、可复现的设计规范：

- **研究一回答“何时显示、显示多久、何时升级”**，形成单层与分层预警的时间参数规范。
- **研究二回答“显示在哪里、空间信息如何随风险变化”**，揭示空间动态信息促进危险定位、风险理解和避险操作的作用机制。
- 最终整合为“**时间分层 × 空间动态编码**”的 AR-HUD 行人碰撞预警时空设计模型。

## 三、核心理论模型

> **版本说明**：2026 年 8 月修订。原"四阶段风险加工链"（对应 PIEV 与 Endsley SA）保留在 3.3，作为本节新框架的上位背景；新增 3.1 的三线夹逼模型与 3.2 的 SPIDER 分解，使时间参数的取值可以从理论**导出**而非从文献**枚举**。理论依据详见 `时间元素设计参数_专题分析.md` 第 2.3.9–2.3.11、2.4.3–2.4.6 与 3.7–3.8 节。

### 3.1 时间参数的三线夹逼模型

预警呈现时刻 $t_{warn}$ 不是一个待搜索的自由参数，而是被三条约束线夹逼、并以自发察觉时刻 $t_0$ 为零点的量。
...
| **中线**：认知加工窗口 | $\Delta t \ge \sum \text{SPIDER}$，**本研究估计** 0.9–1.2 s（非 SPIDER 原文参数，见 §14.2） | 驾驶员的注意与决策耗时 | 实验 1 以 $\Delta t$ 为自变量直接检验 |
| **零点**：自发察觉基线 | $t_0$ | 场景可见性 + 个体差异 | 实验 0 实测；τ 理论给出下限校验 |

三条线的交集即**可行设计区间**。本论文的贡献之一即在于首次把这一区间实测出来，而不是引用其他研究在别的车速下报告的秒数。
...
> **版本说明**：2026 年 8 月修订。相较修订前的四实验结构，本版新增**实验 0（基线标定）**，并把实验 1 的自变量由"绝对 TTC 出现时机"改为"**相对提前量 $\Delta t$ × 触发准则类型**"，把实验 2 新增"**系统可靠性**"维度。修订理由见 §四之附注与 `优化方案_预警时间参数理论化重构与执行计划.md`。

| 研究 | 实验 | 核心问题 | 核心变量及水平 | 主要机制指标（SPIDER 阶段） |
|---|---|---|---|---|
| 研究零：基线标定 | **实验0：无预警基线**（新增） | 驾驶员自己什么时候发现行人？反应多快？ | 车速（40/60 km/h）× 遮挡（无/有），**无 AR 预警** | $t_0$ 分布、PRT 分布、$a_{req}$ 轨迹、系统渲染延迟 |
| 研究一：时间参数设计规范 | 实验1：单层碰撞预警 | 预警相对驾驶员自发察觉应提前多少？触发准则用时间还是减速度？ | **主设计**：触发准则（固定时间阈值 / BTN）× 相对提前量 $\Delta t$（0 / +1.0 / +2.5 s）= 2×3；<br>**区块B**：持续与撤销策略 3 水平（固定 1.5 s / 状态维持 / 风险耦合衰减） | S：TTFF（警告）、TTFF（行人）；<br>E：制动启动、峰值减速度、jerk；<br>安全：最小 TTMD |
| | 实验2：分层碰撞预警 | 两级警告何时升级？系统可靠性如何调节？ | 级间间隔（0.7 / 1.0 / 1.5 s）× 系统可靠性（100% / 80%）= 3×2，另加"无警告基线"与"实验1入选单层方案"两对照 | S：警告→首次注视；<br>I+D：首次注视→正确反应；<br>信任量表、L1 依从率 |
| 研究二：空间动态信息促进作用 | 实验3：空间参照系与锁定策略 | 警告应锁定屏幕、道路、行人还是复合对象？ | 空间参照系与锁定策略 5 水平——Baseline、BD、BR、BW、BW+BR（时间方案固定为实验1–2入选组合） | S：真实行人 TTFF；<br>I：定位转换成本 |
| | 实验4：风险演化动态 | 哪一种连续变化帮助理解风险？ | 2×2 被试内：风险量级空间映射（无/有，**映射量统一为 BTN**）× 运动趋势空间映射（无/有），形成 D0、D1、D2、D3 | P：路径侵入预测；<br>P：冲突点误差 |

**表注**：核心变量指每个实验主动操纵的自变量及其取值。主要机制指标不是核心自变量，也不是全部结果指标，而是用于判断实验操纵通过 SPIDER 哪个阶段发挥作用的过程性因变量（阶段定义见 §3.2）。$\Delta t$ 为相对提前量（= 自发察觉时刻 − 预警时刻），由实验 0 标定；BTN 为制动威胁数（= 避险所需减速度 / 最大可用减速度）；TTFF 为首次注视时间；TTMD 为到达预测最小距离的时间。

**为什么必须先做实验 0**：若不测 $t_0$，则实验 1 的三个水平只能写成绝对 TTC，而绝对 TTC 在 40 km/h 与 60 km/h 下对应完全不同的运动学难度与信息增益。这正是既有文献结论互相冲突的方法学根源——Kang 等（2016，#70）发现 TTC 4 s 优于 2 s，而 Kim 等（2018，#01）发现 TTC 5.0 s 条件下峰值减速度反而上升 34.46%。两者车速（未报告 vs 24 km/h）与模态（听觉 vs 视觉符号）都不同，无法直接比较。用 $\Delta t$ 归一化后，这一冲突才可能被解释。

**为什么实验 2 必须加入系统可靠性**：预警提前量的上界由预测可靠性决定（§3.1 上界线）。若不操纵可靠性，实验只能回答"在完美系统下的最优提前量"，而这在工程上不可实现。本库证据显示：A-TTC（#73，2026 正式版）在 3,519 条视频核验的自然驾驶 FCW 事件上，经动态阈值标定后**事件级骚扰报警占比为 13.74%**（威胁事件召回 89.25%），这是工程上可达的骚扰水平。**该文正式版未报告固定阈值的骚扰报警基线**（其 SSRN 预印本曾报 25.16%，正式版已删除），故本课题不能据此断言真实系统的虚警率水平——20% 这一操纵值的主要依据改为 #78 的最低操纵档（20%）与 #82 的 15% 已证零效应，见 §实验 2。

研究二固定采用研究一按预设决策规则选定的时间方案，使空间实验不再被警告时机差异混淆。实验 4 再固定实验 3 按同一规则选定的空间参照系，形成逐层收敛的研究链条。实验 4 的风险量映射统一采用 BTN，使时间侧与空间侧建立在同一物理量上，便于最终整合为单一时空设计模型。


## 五、研究一：AR-HUD 行人碰撞预警的时间参数设计规范

### 5.1 实验1：单层碰撞预警的时间参数筛选
...
- **$\Delta t$ 的操纵依赖 $t_0$ 的估计精度**。若实验 0 显示 $t_0$ 的被试内标准差 > 1 s，则改用组水平中位数作为锚点，并把个体 $t_0$ 作为协变量入模；此时结论表述须限定为"以组水平自发察觉时刻为基准的提前量"。
...
4. 若置信区间不能区分多个条件，报告"未发现可支持唯一最优值的证据"，并选择实现更简单者进入实验2。
...
- L2 的基准触发点取实验1所得的临界时间（以 $\Delta t$ 表述）；若实验1结果不稳定，预注册 $\Delta t = 0$（即同步确认点）作为 L2 的工程基线。
...
**一手工程依据**——Large et al. (2019) 转述 Daimler 量产级联的三个级间为 1.4 / 1.0 / 0.5 s；Suzuki et al. (2010) 的危险等级每 1.0 s 一档；Lubbe (2017) 实测 2.5 → 1.8 s（即 0.7 s）。三源夹逼出 **0.5–2.0 s** 区间，且存在「越接近碰撞、级间越短」的规律。
...
1. 无警告基线。
2. 实验1按预设规则选定的单层方案（在 100% 可靠性下呈现）。
...
- "提示—验证—紧急执行"的视觉升级规则。
- **在可接受虚警率（20%）下的最早有效预警时刻**，即三线夹逼模型中"上界线"的实测值。
- 系统可靠性退化时的稳健设计建议（对接工程侧的自适应阈值标定，见时间专题分析 3.7.3）。


## 六、研究二：空间动态信息对 AR-HUD 行人碰撞预警效果的促进作用

研究二把“空间动态信息”拆分为两个不同机制：
...
研究一得到的出现时机、持续方式和分层规则在所有 AR 条件中保持一致。不同条件尽量匹配色相、亮度、总视觉面积与信息数量；BW+BR 的新增信息需要通过较低透明度和简化线条控制视觉负担。
...
若 BW 与 BW+BR 的定位收益相当，应优先选择元素更少的 BW；只有 BW+BR 在理解或预测指标上产生具有实际意义的增益且负荷可接受，才进入实验4。

### 6.2 实验4：不同风险演化动态的作用机制
...
为保持因果解释清晰，颜色等级与出现时间固定为研究一的方案；动态条件只改变几何尺度、位置或轨迹，不同时增加文字、声音或新的颜色类别。
...
1. 实验1建立单层预警的时间基线。
2. 实验2将时间基线升级为分层预警，形成“何时提示、何时升级”的规则。
3. 实验3固定时间规则，比较信息“锁定在哪里”，排除时间混淆。
4. 实验4固定时间规则与空间载体，比较信息“如何动态变化”，识别促进机制。
...
- **$t_0$ 与 $\Delta t$**：实验 0 标定，实验 1–4 中作为个体协变量入模。
- **BTN 轨迹**：全程记录 $a_{req}(t)$，用于（a）BTN 触发条件的实现，（b）实验 4 空间编码的映射量，（c）事后计算每个条件对应的运动学难度。
- **系统侧延迟（warning lag）**：实测模拟器从触发条件满足到图形上屏的延迟，须在方法学部分报告。既有 40 篇文献几乎无一报告该量（见时间专题分析 3.8.4）。
- **光学膨胀率 $\dot\theta$**：报告每个条件在预警时刻的 $\dot\theta$ 值，用于判断该条件是否落在知觉可察觉区（$\dot\theta \ge 0.003$ rad/s）内。

### 8.2 “最优设计”的判定顺序

不建议只选择反应时最短的条件。采用约束优先的多指标判定：
...
|---|---|---|---|
| 实验0 | $t_0$、PRT 的分布参数（中位数、p95） | 不适用（无预警基线） | $a_{req}$ 可行域、渲染延迟、晕动脱落率 |
| 实验1 | 制动启动时间与 jerk；准则 × 车速交互 | 碰撞率、最小 TTMD 不劣化 | TTFF、峰值减速度、工作负荷、道路注视比例 |
| 实验2 | 首次注视后到正确反应的时间；可靠性 × 间隔交互 | 最小 TTMD 不劣于最佳单层 | 急刹、漏检、主观干扰、信任量表、L1 依从率 |
| 实验3 | 真实行人 TTFF、目标验证时间 | 目标选择错误与碰撞不增加 | 注视熵、情境意识、负荷 |
| 实验4 | 路径侵入预测准确率、冲突点误差 | 最小 TTMD 与碰撞不劣化 | 紧迫度判断、制动平顺、负荷 |
...
- 可行性：若招募资源有限，可让研究一使用同一批被试完成两个独立 session，研究二使用另一批被试完成两个 session；场景集必须交叉替换，并至少间隔 7 天。
...
| **H2** | $\Delta t = +2.5$ s 相比 $+1.0$ s 在反应时上无显著增益，但在主观负荷与侵入性上更差（收益递减 + 代价递增） | §3.1 上界线 + Wang 等（2025，#20）ARive 的习惯化观察 | "越早越好"成立，上界线需重估 |
| **H3** | BTN 归一化触发在 40 与 60 km/h 两个车速档间的效应一致性显著优于固定时间阈值触发（即准则 × 车速交互显著） | §3.1 下界线 + Takada（#99）、Chen（#72）、Coelingh（#102） | 固定 TTC 阈值已足够，无需引入 BTN |
...
以上均为**待检验假设**。其中 H14 属于跨实验整合推断；只有在实验2和实验4的中介阶段指标均提供一致证据时，才可以作为机制性结论。
...
| BW+BR优于BW | 由目标定位与冲突位置互补推断 | **推断**；必须由实验3直接证据支持 |
| 风险量级动态与轨迹动态作用机制不同 | 理论上分别对应紧迫度理解与路径预测 | **机制假设**；必须由实验4的2×2主效应、交互与过程指标支持 |
...
图题建议：**图1｜AR-HUD行人碰撞预警时空参数的递进研究框架。** 研究一通过实验1和实验2确定单层时间基线与分层升级规则；研究二固定该时间方案，通过实验3和实验4依次检验空间参照系与风险演化动态。四个实验统一映射到感知—理解—预测—执行—安全与代价证据链，最终形成具有适用边界的时空设计规范。图中不包含预设实验结果。
...
flowchart TB
    ...
    subgraph S1[研究一：时间参数设计规范——何时显示、多久、何时升级]
        E1[实验1：单层碰撞预警]
    ...
    subgraph S2[研究二：空间动态信息促进作用——在哪里显示、如何随风险变化]
        E3[实验3：空间参照系动态]
...
flowchart LR
    ...
    subgraph T[研究一：时间参数]
        T1[实验1 单层预警<br/>出现时机 × 持续方式]
        T2[实验2 分层预警<br/>单层/分层 × 级间间隔]
        TO[时间设计规范<br/>何时出现 · 显示多久 · 何时升级]
        T1 --> T2 --> TO
    end

    subgraph D[研究二：空间动态信息]
        D1[实验3 参照系动态<br/>BD / BR / BW / BW+BR]
        D2[实验4 风险演化动态<br/>不变 / 缩放 / 轨迹 / 组合]
...
2. **研究一为研究二提供控制参数**，使论文不是四个并列小实验，而是一条逐步收敛的证据链。
3. **实验3与实验4分别回答“在哪里”和“如何变化”**，把空间动态信息分解为参照系与风险演化两个机制。
4. **实验4使用2×2设计**，不仅能比较不变与动态，还能检验风险尺度动态和轨迹动态是否相互协同。
...
| **SRK 分类学** | 实验 3 对 BR 型「动态驾驶空间」的解释 | Rasmussen (1983) | *IEEE Transactions on Systems, Man, and Cybernetics*，被引 2 318 次 | **高** |
| **生态界面设计** | 分级颜色编码与风险地毯的设计取向 | Vicente & Rasmussen (1992) | 同上，被引 778 次 | **高** |
| **信任量表** | 实验 2 的信任中介测量工具 | Jian et al. (2000) | *International Journal of Cognitive Ergonomics*，被引 1 539 次 | **高**（automation trust 领域标准量表） |
| **报警不信任—依从** | 实验 2 可靠性操纵的理论背景 | Bliss & Acton (2003)；概率匹配见 Bliss et al. (1995) | *Applied Ergonomics*（被引 155）；*Ergonomics*（被引 247） | **中—高**；但「30% 门限」这一具体数值**未在此二文中核实到**，见 §14.9 |
| **虚警 vs 漏报的独立效应** | 「虚警只施加在 L1」的设计依据 | Dixon et al. (2007) | *Human Factors*，被引 215 次 | **高** |
| **S-R 兼容性（危险侧/逃逸侧）** | 实验 3 区块 D 的两个水平 | Straughn et al. (2009) | *IEEE Transactions on Haptics*，被引 69 次；**该文本身即行人碰撞预警研究** | **高**（此前仅经 #100 转引，现已取得一手题录） |
...
§3.2 的预测 P1 与 P2、§10.0 的 H1 与 H4、实验 2 的三个级间间隔水平，共同依赖一个数值：**S + I ≈ 0.9–1.2 s**。追溯其构成：
...
> 「参照 SPIDER 框架（Strayer & Fisher, 2016; Strayer & McDonnell, 2025）对注意阶段的划分，本研究以 Wu et al. (2024) 实测的图标首次注视时间（617 ms）作为扫视阶段的经验估计，并以目标验证时间的推算区间（300–600 ms，本研究由首次注视时间差值估计，原文未直接报告）作为识别阶段的估计，得到最小有效提前量的**工作假设**区间约 0.9–1.2 s。该区间为**本研究的推导值，非文献报告值**，其有效性由实验 1 直接检验。」
...
**一处必须写入方法学的反直觉证据**：Char (2020) 报告 FCW 触发时机由 TTC 2 s 延后 0.3 s 至 1.7 s 后，**制动反应时反而变长**（2 s 条件 0.66–1.28 s；1.7 s 条件 0.93–1.41 s）。这说明 PRT 是**条件依赖量**，不能跨预警时机条件直接比较——直接影响实验 1 各条件间 PRT 的可比性论证。

### 14.5 实验 2：分层预警与系统可靠性
...
| **L2 锚点 TTMD 2.0 s** | Phan (2016) 的覆盖人群百分位表（TTC 1.0 s → 45%、1.6 s → 80%、**2.0 s → 90%**）；Large et al. (2019) 的 PROSPECT 迫近预警 1.8 s；Coelingh et al. (2010) 证明 AEB 物理上最早只能在 1.6 s 介入 | **强**（三源夹逼，均为全文） |
...
| 待核项 | 当前状态 | 影响的变量 |
|---|---|---|
| 不透明度记法（透明度 vs 不透明度）与 0.2／0.6 冲突 | **已由引用链间接裁定**：Ye 与 Yin (2025) 在方法节把 Hou et al. (2024) 的 T 标尺当作「提升**可见性**的 0.75」使用，故 T 功能上是 alpha（不透明度），四源收敛于 **0.6–0.75**；Lopez et al. (2025) 原文为「20% 与 60% **均**支持情境意识」，**并未推荐 20%**，与 ≥ 0.6 不构成冲突。三篇仍仅摘要，**须由原文的参数定义作最终确认**（证据等级：间接） | 实验 3 主条件定为 **0.6**（四源交集的保守下界）；0.2 档保留，但检验目的改为「遮挡收益是否超过可读性代价」 |
...
| 「30% 虚警门限」的原始出处 | 未核实到 | 实验 2 可靠性水平的上界论证 |
| Crompton (1979)、MUTCD、Bennett 等 | 均经 Amini et al. (2019) 转引 | 实验 0 的行人运动学参数 |
| Forkenbrock (2011)、Ueda et al. (2007) | 分别经 Char (2020)、Suzuki et al. (2010) 转引 | PRT 外部基线、$T_{tc,max} = 4.0$ s 的出处 |
| Jing et al. (2022) 全文 | 仅摘要；与 Ye & Yin (2025) 在「虚拟阴影 vs 箭头指向」上直接冲突 | 实验 3 的图形形态选择 |
