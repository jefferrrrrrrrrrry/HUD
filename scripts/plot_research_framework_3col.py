...
block(128, 210, "研究一：驾驶员自发察觉基线的标定（实验 1）", [
    "标定自发察觉时刻 $t_0$、感知反应时 PRT 与 $a_{req}$ 可行域",
    "为实验 2–5 提供时间零点、运动学下界与样本量方差成分",
])

block(372, 210, "研究二：AR-HUD 行人碰撞预警的时间参数设计规范", [
    ("单层预警", "实验 2：相对提前量 $\\Delta t$（0/+1.0/+2.5 s）× 触发准则（时间阈值 / BTN）"),
    ("分层预警", "实验 3：级间间隔（0.7/1.0/1.5 s）× 系统可靠性（100% / 80%）"),
], tagged=True)

block(616, 206, "研究三：空间动态信息对预警效果的促进作用", [
    ("空间参照系", "实验 4：Baseline / BD / BR / BW / BW+BR × 背景视觉复杂度"),
    ("风险演化动态", "实验 5：风险量级映射（BTN）× 运动趋势映射　2 × 2"),
], tagged=True)
