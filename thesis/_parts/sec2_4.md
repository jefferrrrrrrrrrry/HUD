...
**冲突的转化**：实验 2 保留 0.7 / 1.0 / 1.5 s 三水平，但其论证方式从"取自 Lubbe（2017）+ 两侧插值"改为"四源共同界定的区间 0.5–2.0 s 的中段 + 双源支持的 1.0 s 中心值 + 单调缩短规律"。三个水平的理论定位分别是：0.7 s 短于加工窗口（检验融合下界）、1.0 s 恰好覆盖（理论中心）、1.5 s 长于窗口（检验收益递减与"两次独立事件"化）。
...
| 三：级间间隔 0.7 s | A | **加工窗口长度**；紧迫度 | 实验 2 三水平 0.7/1.0/1.5 s，依据改为四源共同界定 |
