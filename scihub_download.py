#!/usr/bin/env python3
"""Batch download missing PDFs via sci-hub.ren / sci.bban.top fallback to multiple mirrors."""
import json
import os
import re
import time
import urllib.parse
import subprocess
import pathlib

ROOT = pathlib.Path("/home/gezhuocheng/HUD")
PAPERS_DIR = ROOT / "papers"
LOG_FILE = ROOT / "scihub_log.json"

PROXY = "http://oversea-squid2.ko.txyun:11080"
UA = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36"

MIRRORS = [
    "sci-hub.ren",
    "www.tesble.com",
    "sci-hub.box",  # ddos-guard but worth trying
    "sci-hub.ru",
    "sci-hub.st",
]

def safe_filename(idx, year, title):
    title_clean = re.sub(r"[^\w\s-]", "", title).strip()
    title_clean = re.sub(r"\s+", "_", title_clean)[:80]
    return f"{idx:02d}_{year}_{title_clean}.pdf"

def curl(url, output=None, follow=True, ua=UA, referer=None, timeout=30):
    cmd = ["curl", "-sk", "--connect-timeout", "10", "--max-time", str(timeout),
           "-A", ua,
           "-x", PROXY]
    if follow:
        cmd.append("-L")
    if referer:
        cmd += ["-H", f"Referer: {referer}"]
    if output:
        cmd += ["-o", output]
    cmd.append(url)
    return subprocess.run(cmd, capture_output=True, text=False)

def fetch_text(url, referer=None, timeout=25):
    r = curl(url, referer=referer, timeout=timeout)
    if r.returncode != 0:
        return None, r.returncode
    try:
        return r.stdout.decode("utf-8", errors="ignore"), 0
    except Exception:
        return r.stdout.decode("latin-1", errors="ignore"), 0

def find_pdf_url(html, mirror):
    """Find PDF embed URL inside sci-hub HTML response."""
    if not html:
        return None
    # Patterns to try
    patterns = [
        r'src="(//[^"]+\.pdf[^"]*)"',
        r'src="(https?://[^"]+\.pdf[^"]*)"',
        r"location\.href='(https?:\\?/\\?/[^']+\.pdf[^']*)'",
        r'embed[^>]+src="([^"]+\.pdf[^"]*)"',
        r'iframe[^>]+src="([^"]+\.pdf[^"]*)"',
    ]
    for pat in patterns:
        m = re.search(pat, html, re.IGNORECASE)
        if m:
            url = m.group(1).replace("\\/", "/")
            if url.startswith("//"):
                url = "https:" + url
            return url
    # If JS challenge / DDoS-Guard - skip
    if "ddos-guard" in html.lower() or "challenge" in html.lower():
        return "DDOS_BLOCKED"
    return None

def download_pdf_url(pdf_url, output_path, referer):
    r = curl(pdf_url, output=output_path, referer=referer, timeout=60)
    if r.returncode != 0:
        return False, f"curl rc={r.returncode}"
    if not os.path.exists(output_path) or os.path.getsize(output_path) < 5000:
        return False, "file too small"
    with open(output_path, "rb") as f:
        sig = f.read(8)
    if not sig.startswith(b"%PDF"):
        os.remove(output_path)
        return False, f"not pdf: {sig[:8]!r}"
    return True, f"OK ({os.path.getsize(output_path)} bytes)"

def try_mirror(doi, mirror):
    url = f"https://{mirror}/{doi}"
    html, _ = fetch_text(url)
    if not html:
        return None, "fetch fail"
    pdf_url = find_pdf_url(html, mirror)
    if pdf_url == "DDOS_BLOCKED":
        return None, "ddos blocked"
    if not pdf_url:
        # Maybe sci-hub returned empty / no article found
        if "article not found" in html.lower():
            return None, "article not found"
        if "<title>Sci-Hub:" in html and len(html) < 2000:
            return None, "empty response"
        return None, "no pdf url"
    return pdf_url, "ok"

def main():
    with open(ROOT / "papers_metadata.json") as f:
        meta = json.load(f)
    downloaded = {1, 3, 4, 6, 13, 17, 20, 21, 22, 25, 26, 29, 33, 34, 35, 37, 38, 39}
    missing = [p for p in meta if p["idx"] not in downloaded]
    print(f"Missing: {len(missing)}")

    log = {}
    if LOG_FILE.exists():
        with open(LOG_FILE) as f:
            log = json.load(f)

    for p in missing:
        idx = p["idx"]
        doi = p["doi"]
        if str(idx) in log and log[str(idx)].get("success"):
            print(f"[{idx:02d}] already done")
            continue
        fname = safe_filename(idx, p["year"], p["title"])
        out = PAPERS_DIR / fname
        if out.exists() and out.stat().st_size > 50000:
            print(f"[{idx:02d}] file exists: {fname}")
            log[str(idx)] = {"success": True, "doi": doi, "file": fname, "via": "existing"}
            continue
        print(f"\n[{idx:02d}] DOI={doi}")
        success = False
        attempts = []
        for mirror in MIRRORS:
            pdf_url, err = try_mirror(doi, mirror)
            attempts.append({"mirror": mirror, "err": err})
            if not pdf_url:
                print(f"  {mirror}: {err}")
                continue
            print(f"  {mirror} -> {pdf_url}")
            ok, msg = download_pdf_url(pdf_url, str(out), referer=f"https://{mirror}/")
            attempts[-1]["dl"] = msg
            if ok:
                print(f"  ✓ Downloaded {msg}")
                log[str(idx)] = {"success": True, "doi": doi, "file": fname, "via": mirror, "pdf_url": pdf_url}
                success = True
                break
            else:
                print(f"  ✗ Download failed: {msg}")
        if not success:
            log[str(idx)] = {"success": False, "doi": doi, "attempts": attempts}
        with open(LOG_FILE, "w") as f:
            json.dump(log, f, indent=2, ensure_ascii=False)
        time.sleep(1.0)

    print("\n=== Summary ===")
    succ = sum(1 for v in log.values() if v.get("success"))
    print(f"Success: {succ} / {len(log)}")

if __name__ == "__main__":
    main()
